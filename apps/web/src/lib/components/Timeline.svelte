              ...
              <div class="active-block-content">
                <div class="active-stats font-mono">
                  <div class="stat-card name-card">
                    <div class="activity-name-text">{$scrambleMode ? '▪' : (block.icon || '▪')} {$scrambleMode ? scrambleText(block.name) : block.name}</div>
                  </div>
                  <div class="stat-card">
...
<style>
  ...
  .block.active::before {
    ...
    position: absolute;
    inset: 0;
    /* Solid dark overlay for incomplete portion - sits on right side */
    background: linear-gradient(
      to right,
      transparent 0%,
      transparent var(--progress-width, 0%),
      rgba(0, 0, 0, 0.75) var(--progress-width, 0%),
      rgba(0, 0, 0, 0.75) 100%
    );
    border-radius: 12px;
    pointer-events: none;
  ...
  .block.active::after {
    ...
    position: absolute;
    inset: 0;
    /* Simple solid color fill for completed portion */
